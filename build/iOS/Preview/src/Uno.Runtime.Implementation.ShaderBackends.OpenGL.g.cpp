...
// /usr/local/share/uno/Packages/UnoCore/0.29.20/Source/Uno/Runtime/Implementation/ShaderBackends/OpenGL/$.uno
// -----------------------------------------------------------------------------------------------------------
...
void GLCompiledProgram::ctor_(uString* vsSource, uString* fsSource, int constCount, int attribCount, uArray* constAttribAndUniformNames)
{
    ...
    for (int i1 = constCount; i1 < (constCount + attribCount); i1++)
        uPtr(_locations)->Item<int>(i1) = ::g::OpenGL::GL::GetAttribLocation(GLProgramHandle(), constAttribAndUniformNames->Strong<uString*>(i1));

    for (int i2 = constCount + attribCount; i2 < constAttribAndUniformNames->Length(); i2++)
        uPtr(_locations)->Item<int>(i2) = ::g::OpenGL::GL::GetUniformLocation(GLProgramHandle(), constAttribAndUniformNames->Strong<uString*>(i2));
}
