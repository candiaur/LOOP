...
Observable.prototype.onValueChanged = function(callback)
{
	this.addSubscriber(function(obs, cmd, value) { 
		if (cmd == "set")
		{
			callback(value); 	
		}
...
Observable.prototype.addSubscriber = function(s)
{
	...
	if (send)
	{
		...
		{
			messageQueue.push({args: [this, "set", this._values[0]], solo: s});
		}
		else
		{
			messageQueue.push({args: [this, "newAll", this._values.slice(0)], solo: s});
		}
		PumpMessages();
	}
}
...
Observable.prototype.tryRemove = function(x)
{
	...
	if (i != -1)
	{
		...
		this._queueMessage(this, "remove", x, i);
		return true;
	}
	else
	{
...
Observable.prototype.removeAt = function(index)
{
	...
	this._queueMessage(this, "remove", obj, index);
}
...
{
	this._values = [value];
	messageQueue.push({args: [this, "set", value], excluding: excludingObserver});
	PumpMessages();
}
...
var messageQueue = [];
var isProcessingMessages = false;
...
function PumpMessages()
{
	...
	for (var m = 0; m < messageQueue.length; m++)
	{
		var msg = messageQueue[m];
		var obs = msg.args[0];
		var solo = msg.solo;
		var exclude = msg.excluding;

		for (var i = 0; i < obs._subscribers.length; i++)
		{
			var s = obs._subscribers[i];
			if (s === exclude) { continue; }
			if (solo && s !== solo) { continue; }
			s.apply(null, msg.args);
		}		
	}
...
{
	messageQueue.push({args: Array.prototype.slice.call(arguments) });
	PumpMessages();
};
